...
export async function createTokenMintAndAssociatedTokenAccount(
	...
	let token1 = await createMintWithTransferFee(connection, payer, mintAuthority, Keypair.generate(), transferFeeConfig)
...
export async function getUserAndPoolVaultAmount(
	...
	const onwerToken0AccountAddr = getAssociatedTokenAddressSync(token0Mint, owner, false, token0Program)

	const onwerToken1AccountAddr = getAssociatedTokenAddressSync(token1Mint, owner, false, token1Program)
