import { MAX_FEE_BASIS_POINTS, ONE_IN_BASIS_POINTS, TOKEN_PROGRAM_ID } from "@solana/spl-token"
...
export function calculatePreFeeAmount(
	...
	if (transferFeeConfig.transferFeeBasisPoints === 0 || tokenProgram.equals(TOKEN_PROGRAM_ID)) {
		return postFeeAmount
	} else {
		const numerator = postFeeAmount * BigInt(MAX_FEE_BASIS_POINTS)
		const denominator = MAX_FEE_BASIS_POINTS - transferFeeConfig.transferFeeBasisPoints
